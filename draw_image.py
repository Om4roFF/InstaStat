...
def draw_like(img, height_of_likes, font, like):
...
def draw_comment(img, height_of_comments, font, comment):
    draw = ImageDraw.Draw(img)
    if comment == 'Недоступно':
        width_of_comments = 190
        font = ImageFont.truetype("arial.ttf", 28)
    else:
...
def draw_share(img, height_of_share, font, share):
    draw = ImageDraw.Draw(img)
    if share == 'Недоступно':
        width_of_share = 370
        font = ImageFont.truetype("arial.ttf", 28)
...
def draw_notes(img, height_of_notes, font, notes):
    draw = ImageDraw.Draw(img)
    if notes == 'Недоступно':
        width_of_notes = 550
        font = ImageFont.truetype("arial.ttf", 28)
...
def draw_visits(img, height_of_visits, font, visits):
    draw = ImageDraw.Draw(img)
    for i in range(720):
...
def draw_coverage(img, height_of_coverage, font, coverage):
    draw = ImageDraw.Draw(img)
...
def draw_name(img, height_of_name, font, name, fill):
    draw = ImageDraw.Draw(img)
